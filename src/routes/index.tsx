...
import ncealogoAsset from "@/assets/ncea-mark.png.asset.json";
...
function StaffInfoRow({ Icon, label, value }: { Icon: (p: IconProps) => ReactElement; label: string; value: string }) {
  return (
    <div className="flex items-center gap-2 text-sm text-white/70">
      <span className="inline-flex w-6 h-6 items-center justify-center rounded-md bg-white/5 ring-1 ring-white/10 text-brand-orange">
    ...
    >
      ...
      <div className="relative flex items-center gap-4">
        ...
        <div className="min-w-0">
          ...
          <div className="text-brand-orange text-xs tracking-wider uppercase mt-1 truncate">{member.role}</div>
        </div>
      </div>

      <div className="relative mt-5 pt-5 border-t border-white/5 flex flex-col gap-2">
        <StaffInfoRow Icon={I.Star} label="Возраст" value={String(member.age)} />
        <StaffInfoRow Icon={I.Compass ?? I.Chat} label="Часовой пояс" value={member.timezone} />
      </div>
      ...
      <a
        ...
        className="relative mt-6 inline-flex items-center justify-center gap-2 h-11 px-5 w-full rounded-full gradient-btn font-medium text-sm"
      >
      ...
      <div className="relative mx-auto max-w-7xl px-4 lg:px-8">
        ...
        <div className="mt-8 max-w-md">
          <label className="relative block">
            <span className="sr-only">Поиск по имени или должности</span>
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Поиск по имени или должности…"
              className="w-full h-12 pl-11 pr-4 rounded-full bg-white/5 ring-1 ring-white/10 focus:ring-brand-orange/50 focus:bg-white/10 outline-none text-sm text-white placeholder:text-white/40 transition"
            />
            <I.Chat className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-white/40" />
          </label>
        </div>

        <div className="mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-5">
          {filtered.map((m, i) => (
            <StaffCard key={m.telegram} member={m} index={i} />
          ))}
        </div>

        {filtered.length === 0 && (
          <div className="mt-10 text-center text-white/50 text-sm">Никого не нашли по запросу «{query}».</div>
        )}
      </div>
    </section>
  );
