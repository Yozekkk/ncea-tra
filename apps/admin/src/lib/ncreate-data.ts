...
export async function saveNCreateSection(
  id: number | null,
  values: TablesInsert<"ncreate_home_sections">,
) {
  const query = id
    ? getSupabase().from("ncreate_home_sections").update(values).eq("id", id)
    : getSupabase().from("ncreate_home_sections").insert(values);
...
export async function saveNCreateCard(
  id: number | null,
  values: TablesInsert<"ncreate_home_cards">,
) {
  const query = id
    ? getSupabase().from("ncreate_home_cards").update(values).eq("id", id)
    : getSupabase().from("ncreate_home_cards").insert(values);
  const { error } = await query.select("id").single();
...
export async function saveNCreateCategory(
  id: number | null,
  values: TablesInsert<"ncreate_forum_categories">,
) {
  const query = id
    ? getSupabase().from("ncreate_forum_categories").update(values).eq("id", id)
    : getSupabase().from("ncreate_forum_categories").insert(values);
  const { error } = await query.select("id").single();
...
export async function getNCreateTopics(): Promise<NCreateTopicView[]> {
  const data = await readRows((from, to) =>
    getSupabase()
      .from("ncreate_forum_topics")
      .select(
        "*, profiles!ncreate_forum_topics_author_id_fkey(username), ncreate_forum_categories(name)",
      )
      .is("deleted_at", null)
  ...
  return (data ?? []).map(({ profiles, ncreate_forum_categories, ...topic }) => ({
    ...topic,
    author: profiles?.username ?? "Unknown",
    category: ncreate_forum_categories?.name ?? "Unknown",
  }));
}
...
export async function getNCreatePosts(): Promise<NCreatePostView[]> {
  const data = await readRows((from, to) =>
    getSupabase()
      .from("ncreate_forum_posts")
      .select(
        "*, profiles!ncreate_forum_posts_author_id_fkey(username), ncreate_forum_topics(title)",
      )
      .is("deleted_at", null)
      .order("created_at", { ascending: false })
      .order("id")
  ...
  return (data ?? []).map(({ profiles, ncreate_forum_topics, ...post }) => ({
    ...post,
    author: profiles?.username ?? "Unknown",
    topic: ncreate_forum_topics?.title ?? "Unknown",
  }));
}
...
export async function deleteNCreateTopic(id: string) {
  ...
  await updateNCreateTopic(id, {
    deleted_at: new Date().toISOString(),
    deleted_by: userId,
    deletion_reason: "Moderated in NCEA Admin / NCreate",
  });
}
