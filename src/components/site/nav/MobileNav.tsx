    ...
    <div className="nv-sheet lg:hidden" role="dialog" aria-modal="true" aria-label="Мобильное меню">
      <div className="nv-sheet-bar">
        <Link to="/" onClick={onClose} className="flex min-w-0 items-center gap-2.5">
          <img src={LOGO_MARK} alt="" width={30} height={30} className="h-[30px] w-[30px] shrink-0 rounded-lg object-cover ring-1 ring-white/12" />
          <span className="truncate font-display text-sm font-extrabold">NCEA <span className="gradient-text">Event Agency</span></span>
        </Link>
      ...
      <div className="nv-sheet-body">
                  ...
                  <div className="nv-sub">
                    {GROUPS.map((group) => (
                      <div key={group.id} className="py-1">
                        <div className="px-2 py-1 text-[11px] font-semibold uppercase tracking-[.14em] text-white/32">{group.label}</div>
                        {SERVICES.filter((service) => service.group === group.id).map((service) => {
                          const Icon = I[service.icon];
                          return (
                            <Link key={service.id} to={service.path} onClick={onClose} className="nv-sub-item">
                    ...
                    <Link to="/services" onClick={onClose} className="nv-cta mt-1 h-11 w-full justify-center rounded-xl text-sm">
                      Все услуги
                    </Link>
                  </div>
                )}
        ...
        <Link to="/services" onClick={onClose} className="nv-cta mt-2 h-12 justify-center rounded-2xl text-sm">
          Рассчитать стоимость <I.Arrow className="h-4 w-4" />
        </Link>
        <div className="grid grid-cols-2 gap-2">
          <a href={DISCORD_URL} target="_blank" rel="noreferrer" className="nv-ghost h-12 rounded-2xl"><I.Discord className="h-5 w-5" /> Discord</a>
          <a href={TELEGRAM_URL} target="_blank" rel="noreferrer" className="nv-ghost h-12 rounded-2xl"><I.Telegram className="h-5 w-5" /> Telegram</a>
        </div>
      </div>
    </div>
  );
